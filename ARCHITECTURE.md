...
flowchart TB
    ...
    TG <-->|"text input/replies + deep links"| GW
    GW --> SE
    GW --> AL
    SE <-->|"state + events"| MEM
    AL <--> MEM
...
pairing via QR. The Gateway owns the PWA's HTTP, WebSocket, session, reconnect, replay, presence,
multi-device, and web-push behavior directly. Messenger Bridges are additive post-v1 modules: the
first concrete Bridge project defines an extension seam for later messenger providers without
forcing the PWA through the same transport lifecycle
([ADR-0008](docs/adr/0008-vps-passkey-byok.md),
[ADR-0028](docs/adr/0028-concrete-bridge-extension-seam.md)). A ChatGPT subscription Model
connection also starts the exactly pinned Codex App Server child shown above; BYOK connections
remain inside the daemon process.

Inside that concrete transport, one React-independent **PWA live-state runtime** owns connection
lifecycle, validated frames, client identity, reconnect/replay cursors, snapshot recovery, buffers,
caches, outbound retry queues, and reconciliation of the client projection. React subscribes to one
immutable snapshot and invokes typed commands; it owns routes, rendering, focus, motion, and local
...
queue only after durable Gateway acceptance; live token deltas are transient, while lifecycle and
final outcomes converge across clients and reconnects
([ADR-0029](docs/adr/0029-gateway-owned-chat-timelines.md)).
...
identity; success and observers occur only after the append-only Event is durably delivered. A
