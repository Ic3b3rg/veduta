 ...
 * characters (see `forbidden-unicode.ts`) from untrusted text BEFORE it is even
...
function normalizeText(input: string): string {
  return stripForbiddenUnicode(input).replace(CONTROL_RE, '').replace(/\s+/g, ' ').trim()
}
...
function delimitedField(name: string, value: string): string {
  ...
  // prompt, not only from what the model hands back. `stripForbiddenUnicode` is
  ...
  const withoutHiddenChars = stripForbiddenUnicode(value)
