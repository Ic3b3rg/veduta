...
/**
 * Whether an "effective selection" exists: either the stored
 * `connections.json` selection points at a connection that is actually
 * `connected`, or — for an install that never went through the Model
 * connections wizard step at all (a legacy BYOK install migrated at boot,
 * `docs/adr/0014-…`'s migration deliberately never sets a selection) — the
 * head of `routing.json`'s reasoning tier resolves a real key through
 * `providerKeys`/`connectionKeys`. Either shape means a chat turn actually
...
function hasEffectiveSelection(
  ...
  if (file.selection !== undefined) {
    const connection = file.connections.find(
      (candidate) => candidate.id === file.selection?.connectionId,
    )
