...
off-main ImapFlow implementation remains historical evidence on
[`archive/025-imap-idle-fallback`](https://github.com/Ic3b3rg/veduta/tree/archive/025-imap-idle-fallback).
Compatible state from installations built from that branch may migrate, but IMAP IDLE is not
treated as shipped `main` behavior. A dedicated mailbox used as a conversational Bridge is a
